...
//! Max memory allocated to ord index DB specific cache in bytes.
static constexpr size_t MAX_ORD_INDEX_CACHE{1024_MiB};
...
CacheSizes CalculateCacheSizes(const ArgsManager& args, size_t n_indexes)
{
    ...
    index_sizes.ord_index = std::min(total_cache / 8, args.GetBoolArg("-ordindex", DEFAULT_ORDINDEX) ? MAX_ORD_INDEX_CACHE : 0);
    total_cache -= index_sizes.ord_index;
